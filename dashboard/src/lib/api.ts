...
export const api = {
  ...
  listProviderModels: (provider: string) =>
    apiFetch<{
      provider: string;
      configured: boolean;
      models: { id: string; display_name: string; enabled?: boolean }[];
  ...
  triggerPipeline: (
    repo: string,
    requirements: string,
    opts: { issueNumber?: number; blueprint?: string; autonomy?: string; brainstorm?: boolean } = {},
  ) =>
    apiFetch<{ run_id: string; stage: string; blueprint?: string }>("/pipeline/trigger", {
      ...
      body: JSON.stringify({
        ...
        brainstorm: opts.brainstorm || undefined,
      }),
    }),
